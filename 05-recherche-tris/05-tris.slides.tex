...
\begin{frame}
 ...
 Ces opérations sont pré-définies en Python :
   \begin{itemize}
   \item \code{e in lst} \quad \emph{ou} \quad \code{i = lst.index(e)}
   \item \code{sorted(lst)} \quad \emph{ou} \quad \code{lst.sort()}
   \end{itemize}
...
\begin{frame}
  ...
  \begin{probleme}[recherche]\small
    \textbf{Données:} une liste \code{lst}, un élément \code{x}\\
    \textbf{Question:} est-ce que \code{lst} contient \code{x}? % (si oui, donner sa position)
  \end{probleme}

  \pause
  \vfill

  Algorithme simple : \defterm{recherche exhaustive}
  \begin{itemize}
  \item Pour chaque élément \code{elem} de \code{lst}
    \begin{itemize}
    \item Si \code{elem} vaut \code{x}, répondre \code{True}
    \end{itemize}
  \item À la fin du parcours, répondre \code{False}
  \end{itemize}
...
\begin{frame}<handout:0>[fragile]
  ...
  \begin{itemize}
  \item Pour chaque élément successif \code{elem} de \code{lst}
    \begin{itemize}
    \item Si \code{elem} vaut \code{x}, répondre \code{True}
    \end{itemize}
  \item À la fin du parcours, répondre \code{False}
...
\begin{frame}[fragile]
 ...
 Complexité :
\begin{itemize}
\item 1 comparaison au mieux, \code{len(lst)} au pire\\
...
\begin{frame}<handout:0>[fragile]
  ...
  Version récursive :
  \begin{itemize}
  \item S'il n'y a plus d'élément non vu, répondre \code{False}
  \item Sinon si le premier élément non vu vaut \code{x} répondre \code{True}
  \item Sinon, rechercher \code{x} parmi les éléments suivants
  \end{itemize}
...
\begin{frame}[fragile]
  ...
  Version récursive (\code{d} : indice de début de recherche) :
  \begin{itemize}
  \item S'il n'y a plus d'élément non vu, répondre \code{False}
  \item Sinon si le premier élément non vu vaut \code{x} répondre \code{True}
  \item Sinon, rechercher \code{x} parmi les éléments suivants
  ...
  \item \defterm{Attention :} coût en espace $O(\code{len(lst)})$ !
  \end{itemize}

\end{frame}

% TODO : exo : renvoyer l'indice
...
\begin{frame}
  ...
  \begin{itemize}
  \item Soit \code{f} une recherche en moins de \code{len(lst)-1}
    accès
  \item Soit \code{lst} une liste quelconque d'entiers
    \defterm{positifs}
  \item \code{f(lst, -1)} renvoie forcément \code{False}
  \item Soit \code{i} l'indice d'un élément non vu
  \item Si l'on avait eu \code{-1} dans \code{lst[i]} on aurait aussi eu
    \code{False}\\ $\longrightarrow$ fonction \defterm{incorrecte} !
  \end{itemize}

  \vfill

  \defterm{Conclusion :} si on ne sait rien sur la liste, on ne peut
  pas faire mieux que \code{len(lst)} comparaisons !

  % \vfill

  % \emph{\color{gray}\footnotesize $\star$ L'argument suppose que
  %   \code{f} accède aux éléments dans un ordre fixé, mais on peut
  %   l'adapter même si ce n'est pas le cas}
\end{frame}
...
\begin{frame}
  ...
  \defterm{Échauffement :} je pense à un nombre entre 1 et
  1000000. Devinez-le.
...
\begin{frame}
  ...
  \item Jeu de la devinette
    % (comment deviner un nombre en posant le moins de question
    % possible ?)
...
\begin{frame}<handout:0>
 \frametitle{La recherche dichotomique en action}
\begin{itemize}
...
\only<12->{
    % retirer la moitié:
%     \foreach \p in {5.25, 5.75}
    \node[fill=red,opacity=.5,inner sep=7pt] at (6.25, 0.25) {};
}
...
\begin{frame}
  ...
  Soient \code{lst} une liste croissante, \code{x} un élément,
  \code{g} et \code{d} deux indices
  ...
  \item Tant que l'intervalle $[\code{g}, \code{d}]$ contient au moins
    un élément :
    \begin{itemize}
    \item Calculer \code{m = (g + d) // 2}
    \item Si \code{lst[m] == x}, répondre \code{True}
    \item Si \code{lst[m] < x}, fixer \code{g} à \code{m+1}
    \item Si \code{lst[m] > x}, fixer \code{d} à \code{m-1}
    \end{itemize}
  \end{itemize}
...
\begin{frame}
  ...
  \item Au début du premier tour de boucle, on cherche parmi \code{len(lst)} éléments (\code{d - g} vaut \code{len(lst) - 1})
  ...
  \item Si $d-g \leq 1$, la boucle s'arrête à la fin du tour
    % \item Tout nombre $n$ peut être divisé par deux au plus
    %   $\lfloor \log_2 n \rfloor + 1$ fois avant que le quotient soit
    %   égal à 0\\
    %   \emph{\small\color{gray} (revoir le cours de MPI)}

  %   \medskip

  % \item \defterm{Conclusion :} au pire $\Theta(\log_2 (\code{len(lst)}))$,
  %   au mieux $\Theta(1)$
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \includegraphics[width=\textwidth]{graphe-comparaisons.png}
\end{frame}
